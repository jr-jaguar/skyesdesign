...
<script type="text/javascript">
  jQuery(document).ready(function($) {
    var options = {
      ...
      $SlideEasing          : $JssorEasing$.$EaseOutCubic,                      //[Optional] Specifies easing for right to left animation, default value is $JssorEasing$.$EaseOutQuad
    ...
    var nestedSlidersOptions = {
      ...
      $ThumbnailNavigatorOptions : {                                   //[Optional] Options to specify and enable thumbnail navigator or not
        ...
        $Cols            : 4,                                                   //[Optional] Number of slides to display in the 'slides' container (slideshow would be disabled if the value is greater than 1)
        $ParkingPosition : 0,                                        //[Optional] The offset position to park thumbnail
        $AutoCenter      : 1
      }
    };
    ...
    function ScaleSlider() {
      ...
      if (parentWidth)
        jssor_slider.$ScaleWidth(Math.max(Math.min(parentWidth, {$settings.standard_slider_width|escape:'htmlall':'UTF-8'}), 30)); else
      ...
      <div u="slides" class="main-slides" style="width: {$settings.standard_slider_width|escape:'htmlall':'UTF-8'}px; height: {$settings.standard_slider_height|escape:'htmlall':'UTF-8'}px;">
        {foreach from=$slides item=slide}
          <div class="slide-inner">
            ...
            {if isset($slide.image) && $slide.image}
              <div class="slide-image col-xs-12">
                <div class="slide-image-wrap">
                  ...
                  {if ($settings.standard_extended_settings && $settings.standard_images_gallery) || !$settings.standard_extended_settings}
                    {if isset($slide.images) && $slide.images}
                      <div id="inner-slider-{$slide.info->id|escape:'htmlall':'UTF-8'}" style="width: 1920px;" class="sliders-inner">
                        <div u="slides" class="inner-slides">
                          {foreach from=$slide.images item=img}
                            <div>
                              ...
                              <img class="js-inner-thumbnail-buttons" u="thumb" src="{$link->getImageLink($slide.info->name, $img.id_image, 'small_default')|escape:'htmlall':'UTF-8'}" alt="{$slide.info->name|escape:'htmlall':'UTF-8'}"/>
                            </div>
                          {/foreach}
                        </div>
