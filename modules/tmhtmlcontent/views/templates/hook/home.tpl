	...
	<ul class="tmhtmlcontent-home clearfix row">
		{foreach name=items from=$htmlitems item=hItem}
			{if $hook == 'left' || $hook == 'right'}
				<li class="tmhtmlcontent-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'} {if isset($isMegaHome) && $isMegaHome}{else} col-xs-12 {/if} {$hItem.specific_class|escape:'htmlall':'UTF-8'} {if $hookName =='footer'} col-xs-4{/if} {if $hookName =='home'} wow {if $smarty.foreach.items.iteration % 2} fadeInLeft {else} fadeInRight {/if} {/if}">
					{else}
				<li class="tmhtmlcontent-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'}{if isset($isMegaHome) && $isMegaHome}{else} col-xs-4 {/if} {$hItem.specific_class|escape:'htmlall':'UTF-8'}">
			{/if}
					{if $hItem.url}
