        ...
        <li class="htmlcontent-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'}{if $hookName =='footer'} col-xs-4{/if} {if $hookName =='home'} wow {if $smarty.foreach.items.iteration % 2} fadeInLeft {else} fadeInRight {/if} {/if}">
          ...
          {elseif $hookName =='home'}
            ...
            {if $hItem.image}
              ...
              {/if}
                  <div class="img-wrapper" style="width: {($hItem.image_w|intval + 40) / 154 * 10}%"><img src="{$link->getMediaLink("`$module_dir`img/`$hItem.image`")}" class="item-img" title="{$hItem.title|escape:'htmlall':'UTF-8'}" alt="{$hItem.title|escape:'htmlall':'UTF-8'}" width="{if $hItem.image_w}{$hItem.image_w|intval}{else}100%{/if}" height="{if $hItem.image_h}{$hItem.image_h|intval}{else}100%{/if}"/></div>
              {if $hItem.url}
                </a>
              {/if}
            {/if}
