...
function titleDinamicStyling(el) {
  if(el.length) {
    $(el).each(function(index, el) {
      var innerText = $(el).text();

      $(el).html(makeNewText(innerText));
    });

    function makeNewText(str) {
      return  str.replace(/\//g, '<span class = "dynamic-styled-in-global-js sorry-for-that">/') + '</span>';
    }
  }
}